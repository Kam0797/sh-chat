...
async function handleLogin(req, res, bypassPassword=false) {
  try{
    ...
    if (passwordMatched || bypassPassword) {
      console.log(req.body.loginEmail,' logged in @',new Date().toDateString(),'EXP', process.env.JWT_EXPIRY);
      //
      ...
      if(!user.uemailVerified) { // marking this ; expire tokens
        let tokenExists = false;
        for(const uemail of uemailVerificationTokensMap.values()) {
